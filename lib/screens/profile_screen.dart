...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  try {
    ...
    // Get like count from separate collection
    final likeCountDoc = await _firestore.collection('like_counts').doc(targetUserId).get();
    final likeCountData = likeCountDoc.data();
    final likesCountFromDB = likeCountData?['count'] ?? 0;
    
    setState(() {
      _userProfile = userProfile;
      _likesCount = likesCountFromDB;
    });
  ...
  void _shareProfile() {
    ...
    final profileUrl = 'https://maarav1.github.io/profile/${_userProfile!.uid}';
  ...
  try {
    ...
    final likeCountRef = _firestore.collection('like_counts').doc(targetUserId);

    if (wasLiked) {
      // Unlike: Remove like document
      await likeRef.delete();
      // Update like count in separate collection
      await likeCountRef.set({
    ...
    _showSnackBar('Failed to update like: ${e.toString()}');
  } finally {
  ...
  try {
    ...
    if (result.data['success'] == true) {
      ...
      _showSnackBar(result.data['message'] ?? 'Failed to send request');
    }
  } catch (e) {
    debugPrint('Error sending friend request: $e');
    _showSnackBar('Failed to send friend request: ${e.toString()}');
  } finally {
    setState(() => _isPerformingAction = false);
  }
}
