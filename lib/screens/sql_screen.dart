...
class AScreen extends StatefulWidget {
  ...
  @override
  State<AScreen> createState() => _AIScreenState();
}

class _AIScreenState extends State<AScreen> with AutomaticKeepAliveClientMixin {
    ...
    _inputFocusNode = FocusNode(
      onKeyEvent: (node, event) {
        ...
        if (event is KeyDownEvent &&
          ...
          // Allow normal backspace operation
  ...
  void _cleanOldConversations() async {
    ...
    final oldConversations = _conversationBox.values
            ...
            _messages.addAll(
              messages.map(
                (msg) => ChatMessage(
                  ...
                  thinkingTime: msg.thinkingTimeMs != null
  ...
  bool _isTimeSensitiveQuery(String query) {
    final keywords = [
      ...
      'news', '2024', '2025', '2026', 'update', 'happening now',
      'just happened', 'this week', 'this month', 'this year'
    ];
...
If information might be outdated or you're unsure about recent developments, mention this limitation.''';
    ...
    // IMPORTANT: Add thinking instructions if enabled
    if (_enableThinking) {
      systemPrompt +=
  ...
  Future<void> _saveConversation() async {
    ...
    try {
      ...
      if (conversations.isNotEmpty) {
        ...
        currentConversation = ConversationHive()
          ..id = conversationId
      ...
      final oldMessages = _chatBox.values
  ...
  Future<void> _loadUrl(String url) async {
    ...
    if (url == 'gemini://api') {
      // Reinitialize API key when switching to API mode
      _initializeApiKey();
    ...
    if (_enableThinking) {
      ...
      if (fullResponse.contains(thinkingStartMarker) &&
        ...
        if (startIndex != -1 && endIndex != -1 && endIndex > startIndex) {
          ...
          final thinkingProcess = fullResponse
              .substring(thinkingStart, endIndex)
              .trim();

          final finalResponse = fullResponse
  ...
  Future<void> _sendGeminiMessage() async {
    ...
    try {
      ...
      _streamSubscription = _streamGeminiResponse(
        ...
        (chunk) {
          if (!mounted || _isStreamCancelled) return;

          setState(() {
            // FIX: Prevent duplicate accumulation by using a buffer
            _streamBuffer += chunk;
            accumulatedResponse = _streamBuffer;
            
            if (_enableThinking) {
              // Check if we're in thinking phase
              if (!thinkingDetected && accumulatedResponse.contains('THINKING_START')) {
                ...
                if (parsed.thinkingProcess.isNotEmpty) {
                  ...
                  _currentStreamText = ''; // Clear stream text during thinking
                  
                  // Stop thinking timer when thinking ends
                  if (accumulatedResponse.contains('THINKING_END') && _thinkingStopwatch.isRunning) {
                ...
                // After thinking is complete, show final response
                final parsed = _parseThinkingResponse(accumulatedResponse);
                _currentStreamText = parsed.finalResponse;
              }
            } else {
              // No thinking mode - just show response
        ...
        onDone: () async {
          ...
          final parsedResponse = _parseThinkingResponse(_streamBuffer);
          
          // Only add to messages if we have actual content
          if (parsedResponse.finalResponse.isNotEmpty || _currentStreamText.isNotEmpty) {
            final responseText = parsedResponse.finalResponse.isNotEmpty 
            ...
            setState(() {
              _messages.add(
                ChatMessage(
                  ...
                  isUser: false,
                  timestamp: DateTime.now(),
                  thinkingProcess: parsedResponse.thinkingProcess.isNotEmpty 
                    ? parsedResponse.thinkingProcess 
                    : _currentThinkingProcess.isNotEmpty
                      ? _currentThinkingProcess
                      : null,
                  thinkingTime: (parsedResponse.thinkingProcess.isNotEmpty || _currentThinkingProcess.isNotEmpty) 
          ...
          // Keep keyboard focused for next message
          WidgetsBinding.instance.addPostFrameCallback((_) {
            if (_inputFocusNode?.hasFocus == false && mounted) {
              _inputFocusNode?.requestFocus();
            }
                      ...
                      GestureDetector(
                        onTap: () {
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(
                              ...
                              backgroundColor:
                                  _enableThinking ? Colors.blueGrey : Colors.blue,
                              duration: const Duration(seconds: 2),
                            ),
                          );
                        },
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(12),
                            border: Border.all(
                              color: _enableThinking ? Colors.lightBlue : Colors.blue,
                              width: 1,
                            ),
                          ),
                          child: Row(
                            children: [
                              Icon(
                                ...
                                size: 10,
                                color:
                                    _enableThinking ? Colors.deepOrangeAccent : Colors.blue,
                              ),
                              ...
                              Text(
                                ...
                                style: TextStyle(
                                  color:
                                      _enableThinking ? Colors.deepOrange : Colors.blue,
                                  fontSize: 10,
                                  fontWeight: FontWeight.bold,
                                ),
                              ),
                  ...
                  child: _messages.isEmpty && _currentStreamText.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemCount: _messages.length +
                              (_currentStreamText.isNotEmpty || _isThinkingPhase ? 1 : 0),
                          itemBuilder: (context, index) {
                            if (index < _messages.length) {
                              ...
                              // Show thinking or streaming message
                            ...
                            Padding(
                              ...
                              child: IconButton(
                                ...
                                onPressed: () async {
                                  ...
                                  if (_messageController.text != currentText) {
                          ...
                          Expanded(
                            child: Container(
                              ...
                              child: TextField(
                                ...
                                onChanged: (value) {
                                  // This ensures the text field stays responsive
                                },
                                onSubmitted: (_) {
                                  _sendGeminiMessage();
